...
%
% Usage
%   >> [mask, tScore, pValue, surroMassOfCluster] = clusterLevelPermutationTest(input1, input2, repeatedMeasuresFlag, pValForPreselection, numIterations)
% 
% Input
...
% History
% 10/19/2020 Makoto. Added help description for the output 'surroMassOfCluster'.
% 06/28/2020 Makoto. 
...
function [mask, tScore, pValue, surroMassOfCluster] = clusterLevelPermutationTest_oneside(input1, input2, repeatedMeasuresFlag, pValForPreselection, numIterations)
...
if repeatedMeasuresFlag == 1
    [~, pValues, ~, stats] = ttest( input1_2D', input2_2D'); %ttest on columns, so observations x Data
  %  [~, pValues, ~, stats] = ttest( input1_2D'); %One sample
else
...
if any(pvalMask(:)) == 0
    ...
    surroMassOfCluster = zeros(numIterations, 1);
    return
end
...
for iterationIdx = 1:numIterations
    ...
    % Perform t-test
    if repeatedMeasuresFlag == 1 %Flip between conditions keeping subj consistency
        permIdx = randi([0, 1], permSize/2, 1);
        surro1 = []; surro2 = [];
        for p = 1:length(permIdx)
            if permIdx(p) == 0
                surro1 = [surro1 input1_2D(:,p)];
                surro2 = [surro2 input2_2D(:,p)];
            else
                surro1 = [surro1 input2_2D(:,p)];
                surro2 = [surro2 input1_2D(:,p)];                
            end
        end
    ...
    %surro1 = input1_2D .* randsample([1,-1],size(input1_2D,2),'true'); %One sample test
    if     repeatedMeasuresFlag == 1   
        [~, pValuesSurro, ~, statsSurro] = ttest(surro1', surro2');
   %     [~, pValuesSurro, ~, statsSurro] = ttest(surro1'); %One sample
    else
    ...
    tScoreSurroMask = tScoreSurro_2D.* pvalSurroMask;
    % If no significant result in the uncorrected result, exit (to save time).
    ...
    for n = 1:length(blobId)
        currentMask = connectedComponentLabels==blobId(n);
        massOfClusterSurro(n) = sum(sum(currentMask.*tScoreSurro_2D));
    end
    ...
    surroMassOfCluster(iterationIdx,1) = max(abs(massOfClusterSurro));%[min(massOfClusterSurro) max(massOfClusterSurro)];
