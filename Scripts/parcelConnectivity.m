% This function performs Network Contingency Analysis on source-level EEG
% data using the underlying procedures described in:
%
% Sripada, Chandra, et al. "Disrupted network architecture of the resting brain 
% in attention‐deficit/hyperactivity disorder." Human brain mapping 35.9 (2014): 4693-4705.
%
% It additionally utilizes the MRI parcellation map detailed in the below
% paper, using a 2mm voxel resolution:
%
...
% July 2022
% Includes dipole projection code from groupSIFT toolbox
% (https://github.com/sccn/groupSIFT, Makoto Miyakoshi)


userInputFilename = 'TNS_ActiveV3';
userInputPercentage = 0.5;  % subject threshold, lower than in groupSIFT because we want to have a sufficient number of conenctions int he whole network, we're not interested ins pecific conenctions

%% Loading in Data
workingFolder = 'D:\ADHD_TNS\TNSstudy\Data\v3Active\';
allFiles = dir([workingFolder '*.set']);
...
for n = 1:length(allFiles)
    EEG = pop_loadset('filename', allFiles(n).name,'filepath',workingFolder, 'loadmode', 'info');
   % EEG = pop_loadset('filename', allFiles{n},'filepath',workingFolder, 'loadmode', 'info'); %using uigetfile
...
fprintf('/nExtracting dipole locations & information for each subject.../n')
...
fprintf('NOTE: .nii file Transformation matrix starting coordinates are: X = %d, Y = %d, Z = %d/n', ...
...
fprintf('/nFinished estimating parcel probabilities./n')
...
fprintf('/nSubject percentages across parcels plotted./n')
...
for sessionIdx = unique(dipolePairAndMeasureObj.sessionId)'
    ...
    save(['D:\ADHD_TNS\TNSstudy\Data\v3Active\testConn\' subIDList{sessionIdx} '.conn'],...
