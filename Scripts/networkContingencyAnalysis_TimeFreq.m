...
%group1Folder = 'D:\filepath\';
%group1Files = dir([group1Folder '*.conn']);

%Select group 2 data
[group2Files, group2Folder] = uigetfile('*.conn', 'MultiSelect', 'on');
if ~any(group1Folder); disp('Cancelled.'); return; end
%group2Folder = 'D:\filepath\';
%group2Files = dir([group2Folder '*.conn']);
...
load(horzcat(group1Folder, group1Files{1}), '-mat', 'parcelInfo', 'frequencies', 'latencies'); %using uigetfile
%load(horzcat(group1Folder, group1Files(1).name), '-mat', 'parcelInfo', 'frequencies', 'latencies');
...
function [connMatSegGroup, finallySelectedEdgeIdx, dims] = loadConnFiles(groupFiles, groupFolder, toIdx, fromIdx, frequencies, latencies, network1, network2)    
    ...
    %for n = 1:size(groupFiles, 2) %Using uigetfile
    for n = 1:size(groupFiles, 1)
        tic
        %load(horzcat(groupFolder, groupFiles{n}), '-mat'); %using uigetfile
        load(horzcat(groupFolder, groupFiles(n).name), '-mat');
